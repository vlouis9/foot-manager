...
export default async function OnboardingPage() {
  ...
  // Club le plus récent (celui créé par /api/game/new)
  ...
  // Initialiser les joueurs si besoin
  ...
  const myPlayers = (myPlayersRaw ?? []).map((cp: any) => cp.players).filter(Boolean)
  const myPlayerIds = myPlayers.map((p: any) => p.id)

  // Tirer 25 cartes par catégorie
  const DRAFT_CONFIG = [
  ...
  for (const { category, count } of DRAFT_CONFIG) {
    let query = supabase
      ...
      .neq('real_team', club.name)
      .limit(count * 15)

    // N'appliquer le filtre d'exclusion que s'il y a des IDs
    if (myPlayerIds.length > 0) {
      query = query.not('id', 'in', `(${myPlayerIds.join(',')})`)
    }
